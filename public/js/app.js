...
$(document).ready(function () {
  ...
  $('#estimateShippingBtn').click(function () {
    return toggleView('#shippingEstimate');
  });
  $('#addNoteBtn').click(function () {
    return toggleView('#addNote');
  });
  $('#addCouponBtn').click(function () {
    return toggleView('#addCoupon');
  });
  ...
  $('.plus').click(function () {
    ...
    var price = $('.priceInCart').text();
    var newResult = Number(equal) + 1;
    $('.equal').text(newResult);
    equal = newResult;
    var subtotal = Number(equal) * Number(price);
...
document.querySelectorAll(".thumbnail").forEach(function (thumbnail) {
  thumbnail.addEventListener("click", function () {
    ...
    document.querySelectorAll(".thumbnail").forEach(function (tn) {
      return tn.classList.remove("selected");
    });
...
function displayCart(data) {
  try {
    ...
    for (var i in items) {
      ...
      var img = item.img;
      var qty = item.qty;
      var _total = item.total;
      var row = "<div class=\"d-flex align-items-center mb-3\" id = \"".concat(id, "\">\n          <img src=\"image/").concat(img, "\" alt=\"").concat(name, "\" class=\"img-thumbnail\"\n              style=\"width: 50px; height: 50px;\">\n          <div class=\"ms-3\">\n              <p class=\"mb-0\">").concat(name, "</p>\n              <strong>$</strong>\n              <strong class=\"priceInCart\">").concat(price, "</strong>\n          </div>\n          <div class=\"ms-auto d-flex align-items-center\">\n              <button class=\"btn btn-outline-secondary btn-sm minus\" onclick = \"minusItem(this, ").concat(id, " )\">-</button>\n              <span class=\"mx-2 equal\">").concat(qty, "</span>\n              <button class=\"btn btn-outline-secondary btn-sm\" onclick = \"addItem(this, ").concat(id, " )\">+</button>\n          </div>\n      </div>");
      rows += row;
    }
...
function minusItemDetail(self, id) {
  var span = $("#product_id_".concat(id, " .productQty .equal"));
  if (span.text() == 1) {
    removeItem(id);
    return;
  }
...
function addItemDetail(self, id) {
  var span = self.parentElement.querySelector('.equal');
  $.ajax({
    type: "get",
    url: "?c=cart&a=add",
    data: {
    ...
    $('#subtotal').html(total);
    $("#product_id_".concat(id, " .total_of_product span")).html("$".concat(Number(item.total).toFixed(2)));
  });
}
function displayCartDetail(data, id) {
  ...
  // Tìm phần tử span gần nút được nhấn   
  $("#product_id_".concat(id, " .productQty .equal")).html(item.qty);
  $('#subtotal').html(total);
  $("#product_id_".concat(id, " .total_of_product span")).html("$".concat(Number(item.total).toFixed(2)));
}
function removeItem(id) {
  var equal = $("#product_id_".concat(id, " .productQty .equal"));
  var qty = equal.text();
  $("#product_id_".concat(id)).remove();
  $.ajax({
    type: "get",
    url: "?c=cart&a=minus",
    data: {
...
function addWishlist(self, id) {
  ...
  $(self).html("<i class=\"bi bi-heart\" title=\"Remove to Wishlist\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\"\n                  data-bs-delay='{\"show\": 100, \"hide\": 100}'></i> ");
  $('[data-bs-toggle="tooltip"]').tooltip();
  $.ajax({
    type: "get",
...
function removeWishlist(self, id) {
  ...
  $(self).html("<i class=\"bi bi-heart\" title=\"Add to Wishlist\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\"\n                  data-bs-delay='{\"show\": 100, \"hide\": 100}'></i> ");
  $('[data-bs-toggle="tooltip"]').tooltip();
  $.ajax({
    type: "get",
...
function updateInput(response, position) {
  ...
  var json = JSON.parse(response);
  for (var i in json) {
    var row = "<option value=\"".concat(json[i].id, "\">").concat(json[i].name, "</option>");
    rows += row;
  }
  $("#".concat(position)).append(rows);
}
...
handles.forEach(function (handle) {
  ...
  handle.addEventListener("mouseup", function () {
    var currentUrl = new URL(window.location.href);
    var minLabel = document.getElementById("minPriceLabel").textContent.replace("$", '');
    var maxLabel = document.getElementById("maxPriceLabel").textContent.replace("$", '');
